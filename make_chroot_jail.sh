...
if [ "$DISTRO" = SUSE ]; then
  ...
  #APPS="/bin/bash /bin/cp /usr/bin/dircolors /bin/ls /bin/mkdir /bin/mv /bin/rm /bin/rmdir /bin/sh /bin/su /usr/bin/groups /usr/bin/id /usr/bin/ssh /usr/sbin/unix_chkpwd /usr/bin/git /usr/bin/env /bin/find /usr/bin/xargs /bin/tar /bin/gzip /bin/cat /usr/bin/dirname /usr/bin/python /usr/bin/make /usr/bin/gcc /usr/bin/g++ /usr/bin/openssl /usr/bin/printf /usr/bin/as /bin/touch /bin/sed /usr/bin/flock /bin/grep /usr/bin/ld /bin/ping"
  APPS=""
fi
...
if grep -Fq "$CHROOT_USERNAME" /etc/sudoers
then
	echo "User already exist in sudoers file"
else
...
if [ "$DISTRO" = SUSE ]; then
  ...
  echo "No more lib"
  #cp /lib64/libnss_compat.so.2 /lib64/libnsl.so.1 /lib64/libnss_files.so.2 /lib64/libcap.so.1 /lib64/libnss_dns.so.2 ${JAILPATH}/lib64/
  #cp /lib/libnss_compat.so.2 /lib/libnsl.so.1 /lib/libnss_files.so.2 /lib/libcap.so.1 /lib/libnss_dns.so.2 ${JAILPATH}/lib/
  #cp /lib64/libnss_compat.so.2 /lib64/libnss_files.so.2 /lib64/libnss_dns.so.2 /lib64/libxcrypt.so.1 ${JAILPATH}/lib64/
  #cp -r /lib64/security ${JAILPATH}/lib64/
  #cp -rf /usr/lib/* ${JAILPATH}/usr/lib
  #cp -rf /usr/lib64/* ${JAILPATH}/usr/lib64
  #cp -rf /lib64/* ${JAILPATH}/lib64/
  #cp -rf /lib/* ${JAILPATH}/lib/
  #cp -rf /usr/libexec ${JAILPATH}/usr/
  #cp -rf /usr/include ${JAILPATH}/usr/
fi
